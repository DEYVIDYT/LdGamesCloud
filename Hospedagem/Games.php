...
// Seu código para gerar o JSON continua aqui...
...
if (isset($response_data['access_token'])) {
    ...
    die("Erro ao obter o token de acesso: " . ($response_data['message'] ?? 'Resposta inválida da API'));
}
...
function corrigirGeneros($genero) {
    return htmlspecialchars($genero, ENT_QUOTES, 'UTF-8');
}
...
if ($name) {
    ...
    $query = "fields name, genres, rating, cover.url, artworks.url, storyline, summary, first_release_date, screenshots.url, videos.video_id; search \"$name\"; limit 20;";
    ...
    foreach ($jogos as $jogo) {
        $jogo_data = [
            ...
            'platform' => "PC",
            'game_link' => "https://thegamesdb.net/game.php?id=" . ($jogo['id'] ?? ''),
            'description' => $jogo['summary'] ?? $jogo['storyline'] ?? null,
            ...
            'video_url' => isset($jogo['videos'][0]['video_id']) ? 'https://www.youtube.com/embed/' . $jogo['videos'][0]['video_id'] : null,
            'banner_url' => isset($jogo['artworks'][0]['url']) ? str_replace('t_thumb', 't_1080p', "https:" . $jogo['artworks'][0]['url']) : null
        ];
    ...
    foreach ($generos as $genero_nome => $genero_id) {
        $query = "fields name, genres, rating, cover.url, artworks.url, storyline, summary, first_release_date, screenshots.url, videos.video_id; where genres = $genero_id; sort rating desc; limit 20;";
        $jogos = buscarJogos($query, $url_igdb, $access_token, $client_id);
